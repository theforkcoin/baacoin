...
class TelegramBotWebHook {
  ...
  constructor(bot) {
    ...
    this._parseBTC = this._parseBTC.bind(this);
  ...
  _parseBTC(error, body) {
    ...
    try {
      this.bot.processButtCoin(data);
    }
    catch (e) {
      console.log('error processing buttcoin', e);
    }
  }
  /**
  ...
  _requestListener(req, res) {
    ...
    if (req.url.indexOf(this.bot.token) !== -1) {
      ...
      res.statusCode = 200;
      res.end('OK');
    } else if (req.url.indexOf('blocktrail') !== -1){
      if (req.method !== 'POST') {
        debug('WebHook request isn\'t a POST');
        res.statusCode = 418; // I'm a teabot!
        res.end();
        ...
        req
          .pipe(bl(this._parseBTC))
          .on('finish', () => res.end('OK'));
      }
    } else {
      debug('WebHook request unauthorized');
      res.statusCode = 200;
      this.bot.processAliveCheck(res);
    }
  }
}
